...
namespace Jewellery.Presentation.WebUI.Helpers
{
    public static class AuthenticationHelper
    {
        ...
        {
            var userDataObject = new CustomFormsAuthenticationUserData(tokenString, account.Role);

            var userData = JsonConvert.SerializeObject(userDataObject);

            return new FormsAuthenticationTicket(1, account.FullName, DateTime.UtcNow, DateTime.UtcNow.Add(FormsAuthentication.Timeout), isPersistent, userData, FormsAuthentication.FormsCookiePath);
        }
